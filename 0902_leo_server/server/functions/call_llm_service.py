...
def _load_llm_config() -> Dict[str, Any]:
    """从 configs/llm.json 加载 llm_service_config"""
    current_dir = Path(__file__).parent  # server/functions
    configs_dir = current_dir.parent / "configs"
    llm_path = configs_dir / "llm.json"
    if not llm_path.exists():
    ...
    try:
        with open(llm_path, "r", encoding="utf-8") as f:
            cfg = json.load(f)
        return cfg.get("llm_service_config", {}) or {}
    except Exception:
        return {}
    ...
    try:
        ...
        if model_config:
            ...
            if llm_config and llm_config.get("api_key"):
                provider = (llm_config.get("provider") or "qwen").strip().lower()
                api_key = llm_config.get("api_key") or ""
                base_url = (llm_config.get("base_url") or "https://dashscope.aliyuncs.com/compatible-mode/v1").rstrip("/")
                model = llm_config.get("model") or "qwen-plus"
            else:
                # 回退到环境变量
